...
<template>
  <div class="academic-manager">
    ...
    <div class="academic-summary-grid">
      <Card v-for="stat in [
        { label: 'Tổng số ' + currentOption.label.toLowerCase(), value: totalRows, icon: 'pi-folder' },
        { label: 'Đang hoạt động', value: activeCount, icon: 'pi-check-circle' },
        { label: 'Cập nhật gần nhất', value: formatRelative(lastUpdatedAt), icon: 'pi-clock' },
      ]" :key="stat.label" class="academic-summary-card">
        <template #content>
          <div class="academic-summary-content">
            <div>
    ...
    <section class="academic-data-card">
      
      <!-- Toolbar & Filters -->
      <Card class="academic-filter-card">
        <template #content>
          <form class="academic-filter-form" @submit.prevent="fetchRows(1)">
            <IconField class="academic-search">
            ...
            <FloatLabel variant="on" class="academic-status-filter">
              <Select
            ...
            <Button type="submit" label="Tìm" icon="pi pi-search" />
            <Button icon="pi pi-refresh" severity="secondary" outlined :loading="loading" aria-label="Làm mới" @click="fetchRows(currentPage)" />
            <Button v-if="!readonly" :label="`Thêm ${currentOption.label.toLowerCase()}`" icon="pi pi-plus" @click="openCreate" />
            <template v-if="isTreeView">
              <Button label="Mở tất cả" icon="pi pi-angle-double-down" severity="secondary" outlined @click="expandAllUnits" />
              <Button label="Thu gọn" icon="pi pi-angle-double-up" severity="secondary" outlined @click="collapseAllUnits" />
            </template>
          </form>
        </template>
      </Card>
