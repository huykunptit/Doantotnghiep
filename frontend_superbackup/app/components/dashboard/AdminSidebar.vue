...
onMounted(() => {
  if (import.meta.client) {
    mobileMedia = window.matchMedia('(max-width: 1080px)')
...
<template>
  <aside
    :class="[
      'sb',
      effectiveCollapsed ? 'sb--collapsed' : '',
    ]"
  >
    <!-- Logo -->
    <div class="sb-brand">
      <NuxtLink to="/admin" class="sb-brand-link">
        <img v-if="siteLogo && !effectiveCollapsed" :src="siteLogo" :alt="siteName" class="sb-brand-img" />
        <img v-else-if="siteLogo && effectiveCollapsed" :src="siteLogo" :alt="siteName" class="sb-brand-favicon" />
    ...
    <div :class="['sb-nav', effectiveCollapsed ? '' : 'sb-nav--scrollable']">
      <!-- Expanded: PanelMenu -->
      <PanelMenu
      ...
      >
        <template #item="{ item }">
          ...
          >
            <span :class="['sb-link-icon', (item as any).icon, isActive((item as any).to) ? 'text-[var(--color-primary)]' : 'text-[var(--muted)]']" />
          ...
          <div v-else class="sb-group-header">
            <span :class="['sb-link-icon', (item as any).icon, 'text-[var(--muted)]']" />
            <span class="sb-link-label">{{ (item as any).label }}</span>
      ...
      <div v-if="effectiveCollapsed" class="sb-collapsed-nav">
        <div
    ...
    <div class="sb-footer">
      <div class="sb-user">
        ...
        <div v-if="!effectiveCollapsed" class="sb-user-info">
