<template>
  <div class="flex flex-col gap-5">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      <div>
        <p class="text-xs font-bold uppercase tracking-widest text-[var(--muted)] mb-1">Giảng viên &bull; Ngân hàng câu hỏi</p>
        <h1 class="text-2xl font-bold tracking-tight text-[var(--text)]">Ngân hàng câu hỏi</h1>
        <p class="text-sm text-[var(--muted)] mt-0.5">Quản lý câu hỏi và bộ đề theo từng khóa học.</p>
      </div>
        ...
        <table class="w-full text-sm text-left border-collapse">
          <thead>
            <tr class="border-b border-[var(--line)] bg-[var(--surface)] text-[0.72rem] font-bold uppercase tracking-wider text-[var(--muted)]">
              <th class="px-5 py-3 w-12">STT</th>
              <th class="px-5 py-3">Khóa học</th>
              <th class="px-5 py-3">Số bài học</th>
          ...
          <tbody>
            ...
            <tr v-for="(course, idx) in filteredCourses" :key="course.id" class="border-b border-[var(--line)] hover:bg-[var(--surface)] transition-colors">
              <td class="px-5 py-4 text-xs font-semibold text-[var(--muted)]">{{ (qbPage - 1) * qbPerPage + idx + 1 }}</td>
              <td class="px-5 py-4">
