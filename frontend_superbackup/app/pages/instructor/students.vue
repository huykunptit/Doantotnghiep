<template>
  <div class="flex flex-col gap-5">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      <div>
        <p class="text-xs font-bold uppercase tracking-widest text-[var(--muted)] mb-1">Giảng viên &bull; Học viên</p>
        <h1 class="text-2xl font-bold tracking-tight text-[var(--text)]">Quản lý học viên</h1>
        <p class="text-sm text-[var(--muted)] mt-0.5">Xem danh sách học viên, tiến độ và mức độ hoàn thành theo từng khóa học.</p>
      </div>
        ...
        <table class="w-full text-sm text-left border-collapse">
          <thead>
            <tr class="border-b border-[var(--line)] bg-[var(--surface)] text-[0.72rem] font-bold uppercase tracking-wider text-[var(--muted)]">
              ...
              <th class="px-5 py-3">Số học viên</th>
            ...
            <tr v-for="(course, idx) in filteredCourses" :key="course.id" class="border-b border-[var(--line)] hover:bg-[var(--surface)] transition-colors">
              ...
              <td class="px-5 py-4 text-right">
                <div class="flex items-center justify-end gap-1.5">
                  <NuxtLink :to="`/instructor/courses/${course.id}/students`" class="h-7 px-3 rounded-lg border border-[var(--line)] bg-white hover:bg-[var(--surface)] text-[10px] font-bold text-[var(--text)] flex items-center justify-center transition-colors">
                    Chi tiết
                  </NuxtLink>
                </div>
              </td>
