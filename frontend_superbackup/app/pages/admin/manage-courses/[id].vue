...
const deleteTarget = ref<{ type: 'section' | 'lesson'; id: number } | null>(null)
...
async function deleteSection(id: number) {
  deleteTarget.value = { type: 'section', id }
}
...
async function deleteLesson(lessonId: number) {
  deleteTarget.value = { type: 'lesson', id: lessonId }
}
...
async function confirmDelete() {
  ...
  if (!target) return
  try {
    const url = target.type === 'section'
      ? `/sections/${target.id}`
      : `/courses/${courseId}/lessons/${target.id}`
    await useApi(url, { method: 'DELETE', headers: authHeaders() })
    successMessage.value = target.type === 'section' ? 'Đã xóa chương.' : 'Đã xóa học liệu.'
    deleteTarget.value = null
    fetchCurriculum()
  } catch (error: any) {
    errorMessage.value = error?.data?.message || (target.type === 'section' ? 'Lỗi khi xóa chương.' : 'Lỗi khi xóa học liệu.')
  }
}
...
<template>
  <div class="flex flex-col gap-5">
    <header class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
      <div><p class="text-xs font-semibold uppercase tracking-wider text-surface-500">Xây dựng nội dung</p><h1 class="mt-1 text-2xl font-bold text-surface-900 dark:text-surface-0">{{ course?.title || 'Đang tải...' }}</h1><p class="mt-1 text-sm text-surface-500">Sắp xếp chương, bài học và cấu hình học liệu.</p></div>
      <div class="flex flex-wrap gap-2"><Button label="Thêm chương" icon="pi pi-plus" @click="openSectionModal()" /><Button label="Xem khóa học" icon="pi pi-external-link" severity="secondary" outlined @click="openCoursePreview" /><Button label="Quay lại" icon="pi pi-arrow-left" severity="secondary" text @click="navigateTo('/admin/manage-courses')" /></div>
    </header>
    ...
    <ProgressSpinner v-if="loading" class="my-10" />

    <Card v-else-if="sections.length === 0"><template #content><div class="py-10 text-center text-surface-500">Chưa có nội dung nào. Hãy bắt đầu bằng cách thêm chương mới.</div></template></Card>

    <div v-else class="curriculum-builder">
      <Card v-for="(section, index) in sections" :key="section.id" class="section-card">
        <template #content>
        <div class="section-header">
          ...
          <div class="section-actions">
            <Button label="Sửa chương" icon="pi pi-pencil" severity="secondary" text size="small" @click="openSectionModal(section)" />
            <Button label="Xóa chương" icon="pi pi-trash" severity="danger" text size="small" @click="deleteSection(section.id)" />
            <Button label="Thêm hoạt động / tài nguyên" icon="pi pi-plus" size="small" @click="openContentPicker(section.id)" />
          </div>
        </div>
        ...
        <div class="lessons-list">
          ...
          <div v-for="(lesson, lIndex) in section.lessons" :key="lesson.id" class="lesson-item">
            ...
            <div class="lesson-actions">
              <Button icon="pi pi-eye" severity="secondary" text rounded aria-label="Xem thử" @click="openLessonPreview(lesson)" />
              <Button icon="pi pi-pencil" text rounded aria-label="Sửa" @click="openLessonModal(section.id, lesson)" />
              <Button icon="pi pi-trash" severity="danger" text rounded aria-label="Xóa" @click="deleteLesson(lesson.id)" />
            </div>
          </div>
        </div>
        </template>
    ...
    <!-- Section Modal -->
    <Dialog v-model:visible="sectionModalOpen" modal :header="sectionForm.id ? 'Sửa tên chương' : 'Thêm chương mới'" class="w-[min(32rem,calc(100vw-2rem))]">
          ...
          <div class="crud-form-grid">
            <label class="crud-field crud-field-full">
              ...
              <InputText v-model="sectionForm.title" placeholder="Ví dụ: Giới thiệu khóa học" />
            </label>
          </div>
          <div class="crud-modal-foot">
            <Button label="Hủy" severity="secondary" text @click="sectionModalOpen = false" />
            <Button label="Lưu chương" :disabled="!sectionForm.title.trim()" @click="saveSection" />
          </div>
    </Dialog>

    <!-- Content Picker Modal -->
    <Dialog v-model:visible="pickerModalOpen" modal header="Chọn hoạt động hoặc tài nguyên" class="w-[min(58rem,calc(100vw-2rem))]">
          <div class="crud-modal-head">
            <div>
          ...
          <div class="picker-body">
            <div class="picker-toolbar">
              <InputText v-model="pickerSearch" placeholder="Tìm kiếm hoạt động hoặc tài nguyên..." />
    ...
    <!-- Lesson Modal -->
    <Dialog v-model:visible="lessonModalOpen" modal :header="lessonForm.id ? 'Cập nhật học liệu' : 'Tạo học liệu'" class="w-[min(64rem,calc(100vw-2rem))]" content-class="max-h-[75vh] overflow-y-auto">
          <div class="crud-modal-head">
            <div>
          ...
          <div class="crud-modal-foot">
            <Button label="Hủy" severity="secondary" text @click="lessonModalOpen = false" />
            <Button label="Lưu học liệu" :loading="lessonSaving" :disabled="!lessonForm.title.trim()" @click="saveLesson" />
          </div>
    </Dialog>

    <!-- Quick lesson preview modal -->
    <Dialog v-model:visible="previewModalOpen" modal :header="previewLesson?.title || 'Xem thử học liệu'" class="w-[min(70rem,calc(100vw-2rem))]" @hide="closeLessonPreview">
          <div class="crud-modal-head">
            <div>
          ...
          <div class="crud-modal-foot">
            <Button label="Đóng" severity="secondary" text @click="closeLessonPreview" />
            <Button label="Mở trong trang học" icon="pi pi-external-link" @click="previewLesson?.id && navigateTo(`/learn/${courseId}/${previewLesson.id}`)" />
          </div>
    </Dialog>
    <Dialog :visible="!!deleteTarget" modal :header="deleteTarget?.type === 'section' ? 'Xóa chương' : 'Xóa học liệu'" class="w-[min(30rem,calc(100vw-2rem))]" @update:visible="!$event && (deleteTarget = null)">
      <p>{{ deleteTarget?.type === 'section' ? 'Xóa chương này? Các học liệu bên trong phải được xóa trước.' : 'Xóa học liệu này? Hành động không thể hoàn tác.' }}</p>
      <template #footer><Button label="Hủy" severity="secondary" text @click="deleteTarget = null" /><Button label="Xóa" icon="pi pi-trash" severity="danger" @click="confirmDelete" /></template>
    </Dialog>
  </div>
</template>
