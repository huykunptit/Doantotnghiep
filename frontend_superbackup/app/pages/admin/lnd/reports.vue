    ...
    <div v-if="activeTab === 'overview'" class="tab-content">
      ...
      <template v-else-if="overviewData">
        <div class="grid grid-cols-2 gap-3 lg:grid-cols-4">
          <Card v-for="item in [
            ['Tổng ghi danh', overviewData.totals.enrollments],
            ['Bài học hoàn thành', overviewData.totals.completed_lessons],
            ['Tổng sinh viên', overviewData.totals.total_students],
            ['Nguy cơ bỏ học', overviewData.totals.at_risk_students]
          ]" :key="String(item[0])">
            <template #content><small>{{ item[0] }}</small><div class="text-xl font-bold">{{ Number(item[1]).toLocaleString('vi-VN') }}</div></template>
          </Card>
        </div>
      ...
      <div v-else-if="reportData" class="report-layout-grid">
        <Card><template #content>
          <div class="flex flex-wrap gap-8">
            <span>Sĩ số <strong>{{ reportData.stats.total_students }}</strong></span>
            <span>Cần cảnh báo <strong>{{ reportData.stats.at_risk_students }}</strong></span>
            <span>Hoàn thành <strong>{{ reportData.stats.average_completion_rate }}%</strong></span>
          </div>
        </template></Card>
    ...
    <div v-if="activeTab === 'atrisk'" class="tab-content">
      ...
      <div v-else class="flex flex-col gap-4">
        <div class="flex items-center justify-between gap-4 mt-2">
          ...
          <InputText v-model="atRiskSearch" placeholder="Tìm theo tên, mã SV..." />
        </div>
        <Card><template #content>
          <DataTable :value="filteredAtRisk" :loading="atRiskLoading" data-key="id" striped-rows responsive-layout="scroll">
            ...
            <Column field="admin_class" header="Lớp" class="hidden md:table-cell" />
            <Column field="cohort" header="Khóa" class="hidden xl:table-cell" />
            <Column field="enrollment_count" header="Số ghi danh" />
            <Column header="Hoạt động cuối"><template #body="{ data }">{{ daysLabel(data.days_inactive) }}</template></Column>
            <Column header="Mức độ"><template #body="{ data }"><Tag :value="riskLevel(data.days_inactive) === 'high' ? 'Nghiêm trọng' : riskLevel(data.days_inactive) === 'medium' ? 'Cần theo dõi' : 'Chú ý'" :severity="riskLevel(data.days_inactive) === 'high' ? 'danger' : 'warn'" /></template></Column>
            <template #empty><div class="py-10 text-center">Không có học viên nào cần cảnh báo</div></template>
          </DataTable>
        </template></Card>
      </div>
    </div>
