  ...
  <div class="flex flex-col gap-5">
    ...
    <Card><template #content><div class="flex flex-col gap-4">
      <div class="flex flex-wrap items-end gap-2">
        <InputText v-model="searchQuery" placeholder="Tìm theo mã hoặc tên lớp..." @keyup.enter="fetchClasses(1)" />
        <Button label="Tìm kiếm" icon="pi pi-search" severity="secondary" outlined @click="fetchClasses(1)" />
        <Button label="Tạo lớp mới" icon="pi pi-plus" @click="openCreate" />
      </div>
      <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <label class="flex flex-col gap-1">
          <span class="text-[0.68rem] font-bold uppercase tracking-wide text-[var(--muted)]">Khóa học vụ</span>
          <Select v-model="filterCohortId" :options="[{ id: '', name: 'Tất cả khóa' }, ...cohorts]" option-label="name" option-value="id" />
        </label>
    ...
    <Card>
      ...
      >
        ...
        <Column field="code" header="Mã lớp">
          <template #body="{ data }"><span class="mono-code">{{ data.code }}</span></template>
        </Column>
        <Column field="name" header="Tên lớp">
          <template #body="{ data }">
          <div class="flex flex-col gap-0.5">
            <strong>{{ data.name }}</strong>
            <span v-if="data.unit" class="text-xs text-[var(--p-text-muted-color)]">{{ data.unit.name }}</span>
          </div>
          </template>
        </Column>
        <Column header="Khóa / Chương trình" class="hidden md:table-cell">
          <template #body="{ data }">
          <div class="flex flex-col gap-0.5">
            <span class="font-semibold">{{ data.cohort?.name ?? '—' }}</span>
            <small>{{ data.program?.name ?? '—' }}</small>
          </div>
          </template>
        </Column>
        <Column header="Cố vấn" class="hidden lg:table-cell">
          <template #body="{ data }">
          <div v-if="data.advisor" class="flex flex-col">
            <strong>{{ data.advisor.name }}</strong><small>{{ data.advisor.email }}</small>
          </div>
          <span v-else>—</span>
          </template>
        </Column>
        <Column header="Sinh viên">
          <template #body="{ data }"><strong>{{ data.students_count ?? 0 }}</strong> / {{ data.capacity }}</template>
        </Column>
        <Column field="status" header="Trạng thái">
          <template #body="{ data }"><Tag :value="statusLabel(data.status)" :severity="data.status === 'active' ? 'success' : data.status === 'suspended' ? 'warn' : 'secondary'" /></template>
        </Column>
        <Column header="Thao tác" frozen align-frozen="right">
          <template #body="{ data }">
          <div class="flex justify-end gap-1.5">
            <Button label="Ghi danh" icon="pi pi-user-plus" size="small" outlined @click="openEnroll(data)" />
            <Button icon="pi pi-pencil" severity="secondary" text rounded aria-label="Sửa" @click="openEdit(data)" />
            <Button icon="pi pi-trash" severity="danger" text rounded aria-label="Xóa" @click="openDelete(data)" />
          </div>
          </template>
        </Column>
        <template #expansion="{ data }">
          <div class="grid gap-2 p-3 text-sm md:grid-cols-3">
            <span><strong>Đơn vị:</strong> {{ data.unit?.name ?? '—' }}</span>
            <span><strong>Chương trình:</strong> {{ data.program?.name ?? '—' }}</span>
            <span><strong>CTĐT:</strong> {{ data.curriculum?.code ?? 'Chưa gán' }}</span>
            <span><strong>Cố vấn:</strong> {{ data.advisor?.name ?? '—' }}</span>
            <span><strong>Tốt nghiệp dự kiến:</strong> {{ data.expected_graduation_year ?? '—' }}</span>
          </div>
        </template>
        <template #empty>
          <div class="py-10 text-center text-[var(--p-text-muted-color)]">Chưa có lớp hành chính nào</div>
        </template>
      </DataTable>
    ...
    <Dialog
      ...
      :header="selectedClass?.name || 'Cập nhật lớp'"
      :modal="true" :style="{ width: '48rem' }"
    >
      <div class="grid grid-cols-2 gap-4 text-left">
        <label class="flex flex-col gap-1.5">
    ...
    <Dialog
      ...
      :header="`Ghi danh sinh viên - ${selectedClass?.name || ''}`"
      :modal="true" :style="{ width: '56rem' }"
    >
