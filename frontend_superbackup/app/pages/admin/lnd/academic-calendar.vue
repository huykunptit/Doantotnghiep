  ...
  <div class="flex flex-col gap-5">
    ...
    <Card><template #content>
      <div class="flex flex-wrap gap-8 text-sm">
        <span>Năm học <strong>{{ academicYears.length }}</strong></span>
        <span>Học kỳ <strong>{{ totalTermsCount }}</strong></span>
        <span>Hiện hành <strong>{{ currentYearName }}</strong></span>
      </div>
    </template></Card>
    ...
    <Dialog
      ...
      :header="yearModalMode === 'create' ? 'Tạo năm học mới' : `Cập nhật: ${selectedYear?.name}`"
    ...
    <Dialog
      ...
      :header="termModalMode === 'create' ? (termForm.is_supplementary ? 'Tạo kỳ học phụ' : 'Tạo học kỳ chính') : `Cập nhật: ${selectedTerm?.name}`"
