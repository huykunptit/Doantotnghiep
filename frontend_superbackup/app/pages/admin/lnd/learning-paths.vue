  ...
  <div class="flex flex-col gap-5">
    ...
    <Card><template #content><div class="flex flex-col gap-4">
      <div class="flex flex-wrap items-end gap-2">
        <button class="inline-flex items-center gap-2 h-9 px-4 rounded-xl bg-[#1d9e75] hover:bg-[#178762] text-white text-xs font-semibold transition-colors shrink-0 cursor-pointer mr-2" type="button" @click="openCreateModal">
          <i class="pi pi-plus" />
      ...
      <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <label class="flex flex-col gap-1">
          <span class="text-[0.68rem] font-bold uppercase tracking-wide text-[var(--muted)]">Chương trình đào tạo</span>
          <select v-model="selectedProgramId" :disabled="loading" class="h-8 px-2 rounded-lg border border-[var(--line)] bg-white text-sm text-[var(--text)] focus:outline-none focus:border-[#1d9e75] cursor-pointer">
            <option v-for="p in programs" :key="p.id" :value="p.id">{{ p.name }} ({{ p.code }})</option>
          </select>
        </label>

        <label class="flex flex-col gap-1">
          <span class="text-[0.68rem] font-bold uppercase tracking-wide text-[var(--muted)]">Chuyên ngành</span>
          <select v-model="selectedMajorId" :disabled="loading" class="h-8 px-2 rounded-lg border border-[var(--line)] bg-white text-sm text-[var(--text)] focus:outline-none focus:border-[#1d9e75] cursor-pointer">
    ...
    <Dialog
      ...
      :header="modalMode === 'create' ? 'Tạo lộ trình đào tạo mới' : modalMode === 'edit' ? 'Cập nhật lộ trình' : selectedCurriculum?.name"
      modal
      :style="{ width: modalMode === 'view' ? '48rem' : '40rem' }"
    >
            ...
            <div v-else>
              ...
              <div class="crud-form-grid" style="padding-top: 10px">
                <label class="crud-field crud-field-full">
                  ...
                  <InputText v-model="curriculumForm.name" placeholder="Ví dụ: Chương trình đào tạo Kỹ sư CNTT khóa 2026" />
                </label>
                <label class="crud-field crud-field-full">
                  <span>Mã lộ trình *</span>
                  <InputText v-if="modalMode === 'edit'" v-model="curriculumForm.code" disabled placeholder="Ví dụ: CNTT_2026" />
                  <InputText v-else v-model="curriculumForm.code" placeholder="Ví dụ: CNTT_2026" />
                </label>
              <label class="crud-field crud-field-full">
                <span>Chương trình đào tạo *</span>
                <Select v-model="curriculumForm.program_id" :options="programs" option-label="name" option-value="id" placeholder="Chọn chương trình đào tạo" />
              </label>
              <label class="crud-field crud-field-full">
                <span>Ngành học (Chuyên ngành)</span>
                <Select v-model="curriculumForm.major_id" :options="majors" option-label="name" option-value="id" show-clear placeholder="Chương trình chung" />
              </label>
            </div>
          </div>
        </div>
          ...
          <template v-if="modalMode !== 'view'" #footer>
            <Button label="Hủy" severity="secondary" text @click="closeModal" />
            <Button label="Lưu lộ trình" :loading="savingCurriculum" @click="saveCurriculum" />
          </template>
    </Dialog>

    <!-- Add Course Modal -->
    <Dialog v-model:visible="showAddCourseModal" :header="`Thêm môn học vào Học kỳ ${selectedTermNumber}`" modal :style="{ width: '56rem' }">
          ...
          <div class="crud-modal-body picker-body-wrap">
            <div class="picker-top-config" style="display: flex; gap: 20px; align-items: flex-end; margin-bottom: 20px;">
              <label class="crud-field row-layout" style="display:flex; align-items:center; gap:8px;">
                <Checkbox v-model="pickerIsRequired" binary />
            ...
            <div class="picker-search-bar" style="margin: 16px 0;">
              <InputText v-model="courseSearchQuery" placeholder="Tìm kiếm môn học cần thêm theo tên..." class="w-full" />
            </div>
          ...
          <template #footer>
            ...
            <Button label="Hủy" severity="secondary" text @click="showAddCourseModal = false" />
            <Button label="Xác nhận thêm" :loading="addingCourses" :disabled="selectedPickerCourseIds.length === 0" @click="addCoursesToCurriculum" />
          </template>
    </Dialog>
