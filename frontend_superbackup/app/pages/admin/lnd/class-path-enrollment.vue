...
<template>
  <div class="flex flex-col gap-5">
      ...
      <div class="flex flex-col gap-4">
        ...
        <Card><template #content><div class="flex flex-col gap-4">
          <div class="flex flex-wrap items-end gap-2">
            <InputText v-model="enrollListSearchQuery" placeholder="Tìm sinh viên, mã SV..." @keyup.enter="enrollmentsPage = 1; loadEnrollments()" />
            <Button label="Tìm kiếm" icon="pi pi-search" severity="secondary" outlined @click="enrollmentsPage = 1; loadEnrollments()" />
            <Button label="Xóa bằng CSV" icon="pi pi-trash" severity="danger" @click="openBulkDeleteModal" />
            <Button v-if="selectedEnrollmentIds.length > 0" :label="`Hủy ghi danh ${selectedEnrollmentIds.length} SV`" icon="pi pi-trash" severity="danger" outlined @click="deleteSelectedEnrollments" />
          </div>
        ...
        <Card><template #content>
          <DataTable :value="enrollments" :loading="loading" data-key="id" striped-rows responsive-layout="scroll">
            <Column header="">
              <template #header><Checkbox :model-value="selectedEnrollmentIds.length === enrollments.length && enrollments.length > 0" binary @update:model-value="toggleSelectAllEnrollments" /></template>
              <template #body="{ data }"><Checkbox :model-value="selectedEnrollmentIds.includes(data.id)" binary @update:model-value="toggleSelectEnrollment(data.id)" /></template>
            </Column>
            <Column header="Mã SV"><template #body="{ data }"><span class="mono-code">{{ data.user?.student_code }}</span></template></Column>
            <Column header="Sinh viên"><template #body="{ data }">
              <div class="flex flex-col gap-0.5">
                <strong>{{ data.user?.name }}</strong><small>{{ data.user?.email }}</small>
              </div>
            </template>
            </Column>
            <Column header="Học phần"><template #body="{ data }"><span class="block max-w-[220px] truncate">{{ data.course?.title }}</span></template></Column>
            <Column header="Học kỳ" class="hidden md:table-cell"><template #body="{ data }">{{ data.term?.name || '—' }}</template></Column>
            <Column header="Lớp tín chỉ" class="hidden lg:table-cell"><template #body="{ data }">{{ data.class_section?.code || '—' }}</template></Column>
            <Column header="Nguồn"><template #body="{ data }"><Tag :value="formatSource(data.enrollment_source)" severity="info" /></template></Column>
            <Column header="Ngày ghi danh" class="hidden xl:table-cell"><template #body="{ data }">{{ new Date(data.enrolled_at).toLocaleDateString('vi-VN') }}</template></Column>
            <Column header="" frozen align-frozen="right"><template #body="{ data }"><Button icon="pi pi-trash" severity="danger" text rounded aria-label="Hủy ghi danh" @click="deleteOneEnrollment(data.id)" /></template></Column>
            <template #empty><div class="py-10 text-center">Không tìm thấy dữ liệu ghi danh</div></template>
          </DataTable>
    ...
    <Dialog
      ...
      header="Xoá ghi danh bằng tệp CSV"
