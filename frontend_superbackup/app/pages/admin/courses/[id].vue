<template>
  <div class="flex flex-col gap-5">
    <header class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
      <div><p class="text-xs font-semibold uppercase tracking-wider text-surface-500">Khóa học</p><h1 class="mt-1 text-2xl font-bold text-surface-900 dark:text-surface-0">Chi tiết khóa học</h1><p class="mt-1 text-sm text-surface-500">Kiểm tra thông tin và thực hiện kiểm duyệt.</p></div>
      <div class="flex flex-wrap gap-2"><Button label="Quay lại" icon="pi pi-arrow-left" severity="secondary" outlined @click="navigateTo('/admin/courses')" /><Button label="Curriculum" icon="pi pi-book" severity="secondary" outlined @click="navigateTo(`/admin/courses/${courseId}/curriculum`)" /><Button v-if="course?.preview_urls?.course" label="Trang khóa học" icon="pi pi-eye" severity="secondary" @click="navigateTo(course.preview_urls.course)" /><Button v-if="course?.preview_urls?.learn" label="Xem thử bài học" icon="pi pi-play" @click="navigateTo(course.preview_urls.learn)" /></div>
    </header>
    <ProgressSpinner v-if="loading" class="my-12" />
          ...
          <Card><template #content>
            ...
            <div class="review-actions">
              <template v-if="course.status === 'pending_review'">
                <Button label="Duyệt & Xuất bản" icon="pi pi-check-circle" :loading="actionLoading" @click="approveCourse" />
                <div class="form-field">
                  <label>Lý do từ chối (Gửi tới giảng viên)</label>
                  <Textarea v-model="rejectReason" rows="4" placeholder="Nhập lý do chi tiết..." fluid />
                </div>
                <Button label="Từ chối khóa học" icon="pi pi-times-circle" severity="danger" :loading="actionLoading" :disabled="!rejectReason.trim()" @click="rejectCourse" />
              </template>
              <template v-else-if="course.status === 'rejected'">
                <div class="alert-rejected">{{ course.reject_reason || 'Khóa học đã bị từ chối.' }}</div>
                <Button label="Duyệt lại khóa học" icon="pi pi-check-circle" :loading="actionLoading" @click="approveCourse" />
              </template>
              <template v-else>
