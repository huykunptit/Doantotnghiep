...
const startsAtDate = computed({
  get: () => form.startsAt ? new Date(form.startsAt) : null,
  set: (value: Date | null) => { form.startsAt = toLocalInput(value) },
})
const endsAtDate = computed({
  get: () => form.endsAt ? new Date(form.endsAt) : null,
  set: (value: Date | null) => { form.endsAt = toLocalInput(value) },
})
...
<template>
  <div class="flex flex-col gap-5">
    ...
    <div class="create-layout">
      ...
      <!-- Right: step content -->
      <Card class="step-content"><template #content>
        ...
        <section v-if="step === 1" class="step-section">
          ...
          <div class="field-stack">
            <label class="crud-field">
              ...
              <InputText v-model="form.title" placeholder="Ví dụ: Đề thi giữa kỳ Toán cao cấp" />
            </label>
            <label class="crud-field">
              <span>Mô tả</span>
              <Textarea v-model="form.description" rows="3" auto-resize placeholder="Phạm vi kiến thức, lưu ý khi thi..." />
            </label>
          </div>
        </section>
        ...
        <section v-if="step === 2" class="step-section">
          <template v-if="form.type === 'course_final'">
            ...
            <Select v-model="form.courseId" :options="courses" option-label="title" option-value="id" filter placeholder="Chọn khóa học" style="max-width:420px;width:100%" />
          ...
          <div class="field-row">
            <label class="crud-field">
              ...
              <InputNumber v-model="form.passScore" :min="0" :max="100" suffix="%" />
            </label>
            <label class="crud-field">
              <span>Số lần thi tối đa</span>
              <InputNumber v-model="form.maxAttempts" :min="1" :max="99" />
            </label>
          </div>
        </section>
        ...
        <section v-if="step === 3" class="step-section">
          ...
          <div class="field-row">
            <label class="crud-field">
              ...
              <InputNumber v-model="form.duration" :min="1" suffix=" phút" />
            </label>
            <label class="crud-field">
              <span>Ngày giờ bắt đầu</span>
              <DatePicker v-model="startsAtDate" show-time hour-format="24" show-icon fluid />
            </label>
            <label class="crud-field">
              <span>Ngày giờ kết thúc</span>
              <DatePicker v-model="endsAtDate" show-time hour-format="24" show-icon fluid />
            </label>
          </div>
          ...
          <div class="check-row">
            <label class="check-label">
              <Checkbox v-model="form.shuffleQuestions" binary />
              <span>Trộn thứ tự câu hỏi</span>
            </label>
            <label class="check-label">
              <Checkbox v-model="form.shuffleAnswers" binary />
        ...
        <section v-if="step === 4" class="step-section">
          ...
          <div v-else class="bank-grid">
            ...
            >
              <Checkbox :model-value="form.selectedBankIds.includes(bank.id)" binary tabindex="-1" style="pointer-events:none" />
              ...
              <div class="bank-count-list">
                <div v-for="bankId in form.selectedBankIds" :key="bankId" class="bank-count-row">
                  ...
                  <div style="display:flex;align-items:center;gap:0.5rem;">
                    <InputNumber v-model="form.bankCounts[bankId]" :min="1" input-style="width:80px" />
            ...
            <template v-if="form.questionMode === 'random_by_difficulty'">
              ...
              <div style="display:flex;align-items:center;gap:1rem;margin-bottom:1rem;">
                ...
                <label class="crud-field" style="margin:0;flex-direction:row;align-items:center;gap:0.5rem;">
                  ...
                  <InputNumber v-model="form.totalQuestions" :min="1" input-style="width:80px" />
                </label>
              </div>
              <div class="diff-table">
                ...
                <div v-for="l in DIFFICULTY_LEVELS" :key="l" :class="['diff-row', { warning: difficultyWarning[l] }]">
                  ...
                  <label class="diff-pct-cell">
                    <InputNumber v-model="form.difficultyDist[l]" :min="0" :max="100" input-style="width:70px" />
                    <span>%</span>
                  </label>
            ...
            <template v-if="form.questionMode === 'manual'">
              ...
              <div v-else class="question-checklist">
                <template v-for="bankId in form.selectedBankIds" :key="bankId">
                  ...
                  >
                    <Checkbox :model-value="form.manualQuestionIds.includes(q.id)" binary tabindex="-1" style="pointer-events:none;flex-shrink:0" />
        ...
        <section v-if="step === 5" class="step-section">
          ...
          <div class="enroll-toolbar">
            <InputText v-model="userSearch" style="max-width:320px" placeholder="Tìm theo tên hoặc email..." />
            <Select v-model="userFilter" :options="[{label:'Tất cả',value:'all'},{label:'Đã chọn',value:'selected'},{label:'Chưa chọn',value:'available'}]" option-label="label" option-value="value" style="max-width:180px" />
            <div style="display:flex;gap:0.5rem;flex-wrap:wrap;">
              <Button label="Chọn tất cả" size="small" severity="secondary" outlined @click="form.enrollUserIds = allUsers.map(u => u.id)" />
              <Button label="Bỏ chọn" size="small" severity="secondary" text @click="form.enrollUserIds = []" />
            </div>
          </div>
          ...
          <div v-else class="user-checklist">
            ...
            >
              <Checkbox :model-value="form.enrollUserIds.includes(u.id)" binary tabindex="-1" style="pointer-events:none;flex-shrink:0" />
              <div class="user-info">
        ...
        <div class="step-nav">
          ...
          <Button v-else label="Quay lại" icon="pi pi-arrow-left" severity="secondary" outlined @click="goBack" />
