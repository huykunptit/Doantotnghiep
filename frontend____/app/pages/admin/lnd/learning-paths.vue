...
<template>
  <div class="flex flex-col gap-5">
    ...
    <UiKpiCards
      :items="[
        { label: 'Tổng số lộ trình', value: curricula.length, subText: 'Lộ trình khung chương trình học', color: 'primary', icon: 'pi-sitemap' },
        { label: 'Môn trong lộ trình', value: curriculumDetails?.summary?.total_subjects ? (curriculumDetails.summary.total_subjects + ' môn') : '—', subText: activeCurriculum ? ('Lộ trình: ' + activeCurriculum.code) : 'Chọn lộ trình để xem', color: 'success', icon: 'pi-book' },
        { label: 'Tổng số tín chỉ', value: curriculumDetails?.summary ? ((curriculumDetails.summary.total_credits_required || 0) + (curriculumDetails.summary.total_credits_elective || 0) + ' TC') : '—', subText: activeCurriculum ? ('Bắt buộc: ' + (curriculumDetails.summary.total_credits_required || 0) + ' TC') : 'Chọn lộ trình để xem', color: 'info', icon: 'pi-star' },
      ]"
      class="mb-4"
    />
