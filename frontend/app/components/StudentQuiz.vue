    ...
    <div v-else class="exam-shell-inline shadow-premium">
      ...
      <header class="exam-topbar">
        <div class="exam-topbar__title">
          ...
          <div v-if="warnings > 0" class="exam-warning">
            <span class="material-symbols-outlined">warning</span> Cảnh báo giám sát: Rời khỏi màn hình {{ warnings }} lần
          </div>
        </div>
...
function notifyError(detail: string) {
  toast.add({ severity: 'error', summary: 'Lỗi', detail, life: 3500 })
}
...
async function submitQuiz(auto = false) {
  ...
  try {
    ...
    notifyError('Không thể nộp bài, vui lòng thử lại sau.')
  } finally {
...
.exam-warning { display: flex; align-items: center; gap: 0.35rem; margin-top: 0.5rem; color: #dc2626; font-size: 0.8rem; font-weight: 700; }
.exam-warning .material-symbols-outlined { font-size: 1rem; }
