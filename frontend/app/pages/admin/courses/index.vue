...
const pendingCount = computed(
  () => courses.value.filter(item => item.status === 'pending_review').length,
)
  ...
  >
    ...
    <section class="crud-overview-grid">
      <article class="dashboard-card mini-card tone-green">
        <p class="mini-title">Tổng khóa học</p>
        <div class="mini-head">
      ...
      <article class="dashboard-card mini-card tone-amber">
        <p class="mini-title">Chờ duyệt</p>
        <div class="mini-head">
          <strong>{{ pendingCount }}</strong>
          <span>Cần rà soát nội dung</span>
        </div>
      </article>

      <article class="dashboard-card mini-card">
        <p class="mini-title">Hình ảnh</p>
        <div class="mini-head">
          <strong>Preview</strong>
          <span>Hiển thị ảnh trực tiếp từ đường dẫn backend</span>
        </div>
      </article>
    </section>
    ...
    <section class="dashboard-card crud-panel">
      ...
      <div class="crud-toolbar">
        ...
        <button class="crud-primary-btn" type="button" @click="createOpen = true">
          Tạo khóa học
        </button>
      </div>
      ...
      <div class="crud-table-wrap">
        <table class="crud-table">
          ...
          <tbody>
            <tr v-if="loading">
              <td colspan="6" class="crud-empty">Đang tải dữ liệu khóa học...</td>
            </tr>
            <tr v-else-if="courses.length === 0">
              <td colspan="6" class="crud-empty">Chưa có khóa học phù hợp bộ lọc.</td>
            </tr>
            <tr v-for="course in courses" :key="course.id">
              ...
              <td>
                <span class="crud-badge" :class="statusClass(course.status)">
                  {{ statusLabel(course.status) }}
                </span>
              </td>
              <td>{{ course.lessons_count || 0 }} bài · {{ course.enrollments_count || 0 }} học viên</td>
              <td>
                <div class="crud-actions">
                  <button
                    class="action-btn is-view"
                    type="button"
                    @click="navigateTo('/admin/question-bank')"
                  >
                    Ngân hàng câu hỏi
                  </button>
                  <button
                    ...
                    type="button"
                    @click="selectedCourse = course; approveOpen = true"
                  >
                    Phê duyệt
                  </button>
                  <button
                    ...
                    type="button"
                    @click="selectedCourse = course; rejectForm.reason = course.reject_reason || ''; rejectOpen = true"
                  >
                    Từ chối
                  </button>
                </div>
    ...
    <CrudConfirmModal
      ...
      :description="`Xác nhận phê duyệt khóa học ${selectedCourse?.title || ''}? Khóa học sẽ được hiển thị cho học viên.`"
    ...
    <!-- ── Reject modal ── -->
    <Teleport to="body">
      <div v-if="rejectOpen" class="crud-modal-backdrop" @click.self="rejectOpen = false">
        <div class="crud-modal">
          <div class="crud-modal-head">
            <div>
              <p class="section-kicker">Từ chối khóa học</p>
              <h3>{{ selectedCourse?.title }}</h3>
              <p>Nhập lý do từ chối để giảng viên có thể chỉnh sửa và gửi duyệt lại.</p>
            </div>
            <button class="topbar-ghost" type="button" @click="rejectOpen = false">✕</button>
          </div>
          ...
          <div class="crud-form-grid">
            <div class="crud-field crud-field-full">
              ...
              <RichTextEditor
                ...
                placeholder="Ví dụ: Hình ảnh thumbnail chưa đúng chuẩn, nội dung cần bổ sung mô tả rõ hơn."
                enable-images
          ...
          <div class="crud-modal-foot">
            <button class="crud-secondary-btn" type="button" @click="rejectOpen = false">Đóng</button>
            <button class="crud-primary-btn crud-danger-btn" type="button" @click="rejectCourse">
              Xác nhận từ chối
            </button>
          </div>
        </div>
        ...
        <div class="crud-modal crud-modal-wide">
          <div class="crud-modal-head">
            <div>
              ...
              <h3>Tạo khóa học trong admin</h3>
            </div>
          ...
          <div class="crud-form-grid">
            <label class="crud-field">
              ...
              <input
                ...
                type="text"
                placeholder="Ví dụ: Laravel cho người mới bắt đầu"
              >
            </label>

            <label class="crud-field">
              <span>Giá</span>
              <input v-model="createForm.price" type="number" min="0">
            </label>
            ...
            <div class="crud-field crud-field-full">
              ...
              <div class="crud-image-preview">
                <img v-if="createForm.thumbnail" :src="createForm.thumbnail" alt="Course thumbnail">
                <div v-else class="crud-image-fallback">CR</div>
                <div>
                  <input
                    type="file"
                    accept="image/*"
                    @change="thumbnailFile = ($event.target as HTMLInputElement)?.files?.[0] || null"
                  >
          ...
          <div class="crud-modal-foot">
            <button class="crud-secondary-btn" type="button" @click="createOpen = false">Đóng</button>
            <button
