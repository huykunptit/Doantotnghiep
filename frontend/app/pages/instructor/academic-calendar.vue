...
async function loadInstitution() {
  try {
    const res = await useApi<Paginator<{ id: number }>>('/instructor/academic/institutions', {
      query: { per_page: 1 },
    })
...
async function loadYears() {
  loading.value = true
  try {
    const res = await useApi<Paginator<AcademicYear>>('/instructor/academic/academic-years', {
      query: { per_page: 50 },
    })
...
async function loadTerms(yearId: number) {
  ...
  try {
    const res = await useApi<Paginator<Term>>('/instructor/academic/terms', {
...
async function saveYear() {
  ...
  try {
    ...
    if (yearEditing.value) {
      await useApi(`/instructor/academic/academic-years/${yearEditing.value.id}`, { method: 'PUT', body })
      toast.add({ severity: 'success', summary: t('admin.calendar.yearUpdated'), life: 2200 })
    }
    else {
      await useApi('/instructor/academic/academic-years', {
        method: 'POST',
...
function askDeleteYear(year: AcademicYear) {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useApi(`/instructor/academic/academic-years/${year.id}`, { method: 'DELETE' })
...
async function saveTerm() {
  ...
  try {
    ...
    if (termEditing.value) {
      await useApi(`/instructor/academic/terms/${termEditing.value.id}`, { method: 'PUT', body })
      toast.add({ severity: 'success', summary: t('admin.calendar.termUpdated'), life: 2200 })
    }
    else {
      await useApi('/instructor/academic/terms', { method: 'POST', body })
...
function askDeleteTerm(term: Term, yearId: number) {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useApi(`/instructor/academic/terms/${term.id}`, { method: 'DELETE' })
