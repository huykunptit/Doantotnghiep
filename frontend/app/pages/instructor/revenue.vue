<template>
  <div class="flex flex-col gap-5">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      <div>
        <p class="text-xs font-bold uppercase tracking-widest text-[var(--muted)] mb-1">Giảng viên &bull; Tài chính</p>
        <h1 class="text-2xl font-bold tracking-tight text-[var(--text)]">Doanh thu</h1>
        <p class="text-sm text-[var(--muted)] mt-0.5">Theo dõi doanh thu, số đơn hàng và hiệu suất kinh doanh theo từng khóa học.</p>
      </div>
        ...
        <table class="w-full text-sm text-left border-collapse">
          ...
          <tbody>
            ...
            <tr v-else-if="filteredCourses.length === 0">
              <td colspan="7" class="px-5 py-8 text-center text-xs text-[var(--muted)]">Chưa có khóa học.</td>
            </tr>
            <tr v-for="(course, idx) in filteredCourses" :key="course.id" class="border-b border-[var(--line)] hover:bg-[var(--surface)] transition-colors">
              ...
              <td class="px-5 py-4 text-xs text-[var(--text)] font-bold">{{ formatPrice(course.price) }}</td>
              <td class="px-5 py-4">
              ...
              <td class="px-5 py-4 text-right">
                <div class="flex items-center justify-end gap-1.5">
                  <NuxtLink :to="`/instructor/courses/${course.id}/revenue`" class="h-7 px-3 rounded-lg border border-[var(--line)] bg-white hover:bg-[var(--surface)] text-[10px] font-bold text-[var(--text)] flex items-center justify-center transition-colors">
                    Chi tiết
                  </NuxtLink>
                </div>
              </td>
