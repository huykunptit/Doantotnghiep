<template>
  <NuxtLayout name="instructor">
    <section class="space-y-8">
      <AppPageHeader eyebrow="Instructor" title="Đợt thi & đánh giá" description="Quản lý ngân hàng câu hỏi, đề kiểm tra và kỳ thi độc lập theo từng khóa học." />
      ...
      <UiCard>
        ...
        <UiEmptyState v-else-if="filteredCourses.length === 0" title="Chưa có khóa học" description="Tạo khóa học trước khi cấu hình đợt thi và đánh giá." />
              ...
              <tr v-for="(course, idx) in filteredCourses" :key="course.id" class="hover:bg-surface-low/50 transition-colors">
                ...
                <td class="px-5 py-4 text-right">
                  <div class="flex items-center justify-end gap-2">
                    <NuxtLink :to="`/instructor/courses/${course.id}/question-bank`" class="inline-flex items-center gap-1 rounded-lg bg-primary/10 px-2.5 py-1.5 text-xs font-bold text-primary hover:bg-primary/20 transition-colors" title="Ngân hàng câu hỏi">
                      <span class="material-symbols-outlined text-[14px]">database</span> Câu hỏi
                    </NuxtLink>
                    <NuxtLink :to="`/instructor/courses/${course.id}/exams`" class="inline-flex items-center gap-1 rounded-lg bg-secondary/10 px-2.5 py-1.5 text-xs font-bold text-secondary hover:bg-secondary/20 transition-colors" title="Kỳ thi">
                      <span class="material-symbols-outlined text-[14px]">quiz</span> Kỳ thi
                    </NuxtLink>
                  </div>
                </td>
              </tr>
